async function misionHX79() {
	...
	let clima = await consultaClimaHX79(fecha, oceano);
	let conv = convHX79(clima);
	let sub = submarinosHX79();
	let textoMision = textoHX79(fecha, oceano, clima);
	let ordenesMision = ordenesHX79();
...
async function consultaClimaHX79(fecha, oceano) {
...
function convHX79(clima) {
	...
	conv.mercantes = extraerAleatorios(cargos, 41)
		.concat(extraerAleatorios(pasageros, 1),
			extraerAleatorios(tanqueros, 9),
			extraerAleatorios(remolcadores, 6));
...
function textoHX79(fecha, oceano, clima) {
...
Confío en su experiencia y determinación, Kapitänleutnant. Que la fortuna lo acompañe. La Patria espera resultados.`;
	...
	texto.victoria = `El Convoy HX79 fue un convoy que cruzó el Atlántico Norte desde Halifax a Liverpool.
El 17 de Octubre de 1940 fue avistado y atacado en solitario por el U-48.
A la mañana siguiente, el 18 de Octubre, otros cuatro U-boote se sumaron a la cacería.
Fueron hundidos 12 de los 50 mercantes, 1 resulto dañado. Los aliados perdieron 75069 GRT.
Ningún submarino resultó dañado.
79 marineros no volvieron a casa.`;
