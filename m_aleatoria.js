async function misionAleatoria() {
	...
	let clima = await consultaClimaAleatoria(fecha, oceano);
	let conv = genConv(fecha, clima);
	let sub = submarinos();
	let textoMision = texto(fecha, oceano, clima);
	let ordenesMision = ordenes();
...
function texto(fecha, oceano, clima) {
...
async function consultaClimaAleatoria(fecha, oceano) {
