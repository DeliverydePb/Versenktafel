async function misionAleatoria() {
	...
	let oceano = oceanoAleatorio();
	let clima = await consultaClima(fecha, oceano);
	let conv = genConvAleatorio(fecha, clima);
	let sub = submarinosAleatorios();
	let textoMision = textoGenerico(fecha, oceano, clima);
	let ordenesMision = ordenesAleatorias();
...
function genConvAleatorio(fecha, clima) {
	...
	if (aleatorioEntre(0, 0.999, 3) < probCamRumb) { conv.cambRmb = "true"; }
	else { conv.cambRmb = "false"; }
