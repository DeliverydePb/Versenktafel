async function misionHX97() {
	...
	let clima = await consultaClimaHX97(fecha, oceano);
	let conv = convHX97(clima);
	let sub = submarinosHX97();
	let textoMision = textoHX97(fecha, oceano, clima);
	let ordenesMision = ordenesHX97();
...
async function consultaClimaHX97(fecha, oceano) {
...
function convHX97(clima) {
	...
	conv.mercantes = extraerAleatorios(cargos, 21)
		.concat(extraerAleatorios(pasageros, 1),
			extraerAleatorios(tanqueros, 3),
			extraerAleatorios(remolcadores, 0));
...
function textoHX97(fecha, oceano, clima) {
...
Confío en su experiencia y determinación, Kapitänleutnant. Que la fortuna lo acompañe. La Patria espera resultados.`;
	...
	texto.victoria = `El Convoy HX97 zarpó el 18 de diciembre de 1940 desde Halifax hacia Liverpool.
El 29 de diciembre, el carguero Valparaíso quedó rezagado del convoy debido a una avería mecánica.
El último día de 1940 a las 21:12 horas, fue alcanzado por un torpedo G7a del U-38 y se hundió por la popa.
El capitán, 32 tripulantes y dos pasajeros, no volvieron a casa.`;
